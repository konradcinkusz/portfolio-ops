...
from portfolio_ops.report import ReportInput, WorkflowRun, sections
...
def test_work_that_is_all_in_the_plan_is_no_item(build: Build) -> None:
    ...
    assert section.lines == (
        (
            "7 repositories (0 public, 7 private) of example-owner scanned; your activity since "
            "2026-09-15 is in 3 of them."
        ),
        "",
        (
...
def test_a_token_that_sees_no_private_repository_is_an_item(build: Build) -> None:
    public = worked("open-tool", private=False)
    scan = AccountScan(
        "example-owner",
        window_start(TODAY),
    ...
    data = replace(with_repos(build), account=Account(scan=scan))

    section = sections.account_activity(data)
    health = sections.health(data)

    assert section.has_items
    assert section.lines[:3] == (
        (
            "- **The token sees only public repositories:** the token does not list this "
            "private repository, example-owner/portfolio-data, so every private repository of "
            "the account is missing from the scan — on GitHub, edit the token and set its "
            'repository access to "All repositories".'
        ),
        "",
        (
            "1 repository (1 public, 0 private) of example-owner scanned; your activity since "
            "2026-09-15 is in 1 of them."
        ),
    )
    assert (
        "- Account: 1 repository worked on since 2026-09-15, 1 of them outside the portfolio "
        "(public repositories only)" in health.lines
    )


def test_in_github_actions_the_header_links_the_workflow_run(build: Build) -> None:
    run = WorkflowRun("https://github.com/example-owner/portfolio-data/actions/runs/42", "7")

    rendered = render(replace(build(), run=run))

    assert rendered.markdown.splitlines()[2] == (
        "Generated by portfolio-ops for 2026-09-22 (UTC) in [workflow run #7]"
        "(https://github.com/example-owner/portfolio-data/actions/runs/42); the dashboard is "
        "among its artifacts."
    )


def test_in_github_actions_the_header_links_the_overview_too(build: Build) -> None:
    run = WorkflowRun("https://github.com/example-owner/portfolio-data/actions/runs/42", "7")
    home = "https://github.com/example-owner/portfolio-data/blob/HEAD/overview/README.md"

    linked = render(replace(build(), run=run, overview=home))
    alone = render(replace(build(), overview=home))

    assert linked.markdown.splitlines()[2] == (
        "Generated by portfolio-ops for 2026-09-22 (UTC) in [workflow run #7]"
        "(https://github.com/example-owner/portfolio-data/actions/runs/42); the dashboard is "
        f"among its artifacts, and [the overview]({home}) shows the whole portfolio."
    )
    assert alone.markdown.splitlines()[2] == "Generated by portfolio-ops for 2026-09-22 (UTC)."
